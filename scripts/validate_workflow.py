...
class WorkflowValidator:
    ...
    def validate_workflow_files(self):
        ...
        for workflow_file in workflow_files:
            ...
            if not re.search(r"name:", content):
                self.warnings.append(f"{workflow_file.name} is missing a name field")
    ...
    def validate_python_scripts(self):
        ...
        if not python_files:
            self.warnings.append("No Python scripts found")
            return
    ...
    def validate_requirements_file(self):
        ...
        for package in essential_packages:
            if not re.search(fr"{package}[\s=~>]", content):
                self.warnings.append(f"requirements.txt might be missing {package}")
